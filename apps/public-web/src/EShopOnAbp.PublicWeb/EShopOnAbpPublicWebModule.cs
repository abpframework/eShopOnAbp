...
public class EShopOnAbpPublicWebModule : AbpModule
{
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        app.UseConfiguredEndpoints(endpoints =>
        {
            ...
            endpoints.MapForwarder("*/product-images/{name}", "http://catalog:80/product-images", "/{name}");
            endpoints.MapForwarder("/products/product-images/{name}", "http://catalog:80/product-images", "/{name}");
        });
    }
